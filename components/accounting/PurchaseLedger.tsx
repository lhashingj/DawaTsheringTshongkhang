...
import { db, purchaseCRUD, PurchaseRecord, PurchaseItem, UnitType, autoIncrementStock, postPurchaseToGL } from '@/lib/accounting-db';
...
export function PurchaseLedger() {
  ...
  async function saveForm() {
    ...
    if (modalMode === 'add') {
      ...
      // Auto-increment inventory and post double-entry GL (only on new purchase)
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={p.id} className="border-t border-slate-700 hover:bg-slate-700/30 transition-colors">
                ...
                <td className="px-4 py-3 text-right text-slate-300 font-mono">{p.grossAmount.toFixed(2)}</td>
                <td className="px-4 py-3 text-right text-yellow-400 font-mono">{p.gstAmount.toFixed(2)}</td>
                <td className="px-4 py-3 text-right text-orange-400 font-mono font-semibold">{p.netAmount.toFixed(2)}</td>
            ...
            <div className="overflow-x-auto rounded-lg border border-slate-700">
              <table className="w-full text-sm">
                ...
                <tfoot className="border-t-2 border-slate-600">
                  <tr><td colSpan={4} className="px-3 py-2 text-right text-slate-400">Gross</td><td className="px-3 py-2 text-right text-slate-300">{selected.grossAmount.toFixed(2)}</td></tr>
                  <tr><td colSpan={4} className="px-3 py-2 text-right text-slate-400">GST {selected.gstRate}%</td><td className="px-3 py-2 text-right text-yellow-400">{selected.gstAmount.toFixed(2)}</td></tr>
                  <tr><td colSpan={4} className="px-3 py-2 text-right text-white font-bold">Net Amount</td><td className="px-3 py-2 text-right text-orange-400 font-bold">{selected.netAmount.toFixed(2)}</td></tr>
                </tfoot>
              </table>
            </div>
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-slate-400 text-xs mb-1">PO Number</label><input className={inputCls} value={form.purchaseOrderNo} onChange={e => setForm(p => ({ ...p, purchaseOrderNo: e.target.value }))} /></div>
                <div><label className="block text-slate-400 text-xs mb-1">Date</label><input type="date" className={inputCls} value={new Date(form.timestamp).toISOString().split('T')[0]} onChange={e => setForm(p => ({ ...p, timestamp: new Date(e.target.value) }))} /></div>
                <div><label className="block text-slate-400 text-xs mb-1">Supplier Name</label><input className={inputCls} value={form.supplierName} onChange={e => setForm(p => ({ ...p, supplierName: e.target.value }))} /></div>
                <div><label className="block text-slate-400 text-xs mb-1">Phone</label><input className={inputCls} value={form.supplierPhone || ''} onChange={e => setForm(p => ({ ...p, supplierPhone: e.target.value }))} /></div>
                <div><label className="block text-slate-400 text-xs mb-1">Address</label><input className={inputCls} value={form.supplierAddress || ''} onChange={e => setForm(p => ({ ...p, supplierAddress: e.target.value }))} /></div>
                <div><label className="block text-slate-400 text-xs mb-1">TPN</label><input className={inputCls} value={form.supplierTPN || ''} onChange={e => setForm(p => ({ ...p, supplierTPN: e.target.value }))} /></div>
                <div><label className="block text-slate-400 text-xs mb-1">GST Rate %</label><input type="number" className={inputCls} value={form.gstRate} onChange={e => setForm(p => ({ ...p, gstRate: parseFloat(e.target.value) || 0 }))} /></div>
              </div>
                  ...
                  <tbody>
                    {formItems.map((item, i) => (
                      <tr key={i} className="border-t border-slate-700">
                        ...
                        <td className="px-3 py-2 text-right text-slate-300">{item.rate.toFixed(2)}</td>
                        <td className="px-3 py-2 text-right text-orange-400">{item.amount.toFixed(2)}</td>
                        <td className="px-2 py-2"><button onClick={() => setFormItems(p => p.filter((_, j) => j !== i))} className="text-slate-500 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button></td>
                      </tr>
                    ))}
                    <tr className="border-t border-slate-600 bg-slate-700/30">
                      ...
                      <td className="px-2 py-1"><input type="number" className="w-16 bg-slate-700 border border-slate-600 text-white rounded px-2 py-1 text-xs focus:outline-none focus:border-orange-500" value={newItem.qty} onChange={e => setNewItem(p => ({ ...p, qty: e.target.value }))} /></td>
                      <td className="px-2 py-1"><select className="w-full bg-slate-700 border border-slate-600 text-white rounded px-1 py-1 text-xs focus:outline-none" value={newItem.unit} onChange={e => setNewItem(p => ({ ...p, unit: e.target.value as UnitType }))}>{UNITS.map(u => <option key={u} value={u}>{u}</option>)}</select></td>
                      <td className="px-2 py-1"><input type="number" className="w-20 bg-slate-700 border border-slate-600 text-white rounded px-2 py-1 text-xs focus:outline-none focus:border-orange-500" placeholder="Rate" value={newItem.rate} onChange={e => setNewItem(p => ({ ...p, rate: e.target.value }))} /></td>
                      <td />
                      <td className="px-2 py-1"><button onClick={addFormItem} className="text-orange-400 hover:text-orange-300"><Plus className="w-4 h-4" /></button></td>
                    </tr>
                  </tbody>
                  {formItems.length > 0 && (
                    <tfoot className="border-t-2 border-slate-600">
                      {(() => { const t = computeTotals(formItems, form.gstRate); return (<>
                        <tr><td colSpan={5} className="px-3 py-1 text-right text-slate-400 text-xs">Gross: Nu. {t.grossAmount.toFixed(2)}</td><td /></tr>
                        <tr><td colSpan={5} className="px-3 py-1 text-right text-slate-400 text-xs">GST {form.gstRate}%: Nu. {t.gstAmount.toFixed(2)}</td><td /></tr>
                        <tr><td colSpan={5} className="px-3 py-2 text-right text-orange-400 font-semibold text-sm">Net: Nu. {t.netAmount.toFixed(2)}</td><td /></tr>
                      </>); })()}
                    </tfoot>
                  )}
                </table>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={saveForm} disabled={isSaving || !form.supplierName} className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:bg-slate-600 text-white py-2.5 rounded-lg text-sm font-medium transition-colors">{isSaving ? 'Saving…' : 'Save'}</button>
